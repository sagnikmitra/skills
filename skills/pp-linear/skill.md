...
When the user invokes `/pp-linear` or describes a task the skill's description matches.
...
- Claude (if synced from `~/.claude/skills/pp-linear`)
- HQ Project — landing page Skills section
- MD Project (md.sgnk.ai) — `Skills/Pp Linear/`
- Obsidian Vault — `Skills/Pp Linear/`
...
_Source: ~/.codex/skills/pp-linear/SKILL.md_
