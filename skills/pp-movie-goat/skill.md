...
When the user invokes `/pp-movie-goat` or describes a task the skill's description matches.
...
- Claude (if synced from `~/.claude/skills/pp-movie-goat`)
- HQ Project — landing page Skills section
- MD Project (md.sgnk.ai) — `Skills/Pp Movie Goat/`
- Obsidian Vault — `Skills/Pp Movie Goat/`
...
_Source: ~/.codex/skills/pp-movie-goat/SKILL.md_
