...
When the user invokes `/pp-company-goat` or describes a task the skill's description matches.
...
- Claude (if synced from `~/.claude/skills/pp-company-goat`)
- HQ Project — landing page Skills section
- MD Project (md.sgnk.ai) — `Skills/Pp Company Goat/`
- Obsidian Vault — `Skills/Pp Company Goat/`
...
_Source: ~/.codex/skills/pp-company-goat/SKILL.md_
