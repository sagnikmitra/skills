...
When the user invokes `/pp-agent-capture` or describes a task the skill's description matches.
...
- Claude (if synced from `~/.claude/skills/pp-agent-capture`)
- HQ Project — landing page Skills section
- MD Project (md.sgnk.ai) — `Skills/Pp Agent Capture/`
- Obsidian Vault — `Skills/Pp Agent Capture/`
...
_Source: ~/.codex/skills/pp-agent-capture/SKILL.md_
