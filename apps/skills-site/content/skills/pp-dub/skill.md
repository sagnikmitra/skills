...
When the user invokes `/pp-dub` or describes a task the skill's description matches.
...
- Claude (if synced from `~/.claude/skills/pp-dub`)
- HQ Project — landing page Skills section
- MD Project (md.sgnk.ai) — `Skills/Pp Dub/`
- Obsidian Vault — `Skills/Pp Dub/`
...
_Source: ~/.codex/skills/pp-dub/SKILL.md_
