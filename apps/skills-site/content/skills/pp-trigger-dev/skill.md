...
When the user invokes `/pp-trigger-dev` or describes a task the skill's description matches.
...
- Claude (if synced from `~/.claude/skills/pp-trigger-dev`)
- HQ Project — landing page Skills section
- MD Project (md.sgnk.ai) — `Skills/Pp Trigger Dev/`
- Obsidian Vault — `Skills/Pp Trigger Dev/`
...
_Source: ~/.codex/skills/pp-trigger-dev/SKILL.md_
