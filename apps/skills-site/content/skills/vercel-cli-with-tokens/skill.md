...
When the user invokes `/vercel-cli-with-tokens` or describes a task the skill's description matches.
...
- Claude (if synced from `~/.claude/skills/vercel-cli-with-tokens`)
- HQ Project — landing page Skills section
- MD Project (md.sgnk.ai) — `Skills/Vercel Cli With Tokens/`
- Obsidian Vault — `Skills/Vercel Cli With Tokens/`
...
_Source: ~/.claude/skills/vercel-cli-with-tokens/SKILL.md_
_Category: General_
