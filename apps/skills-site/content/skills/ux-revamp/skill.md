...
Travox UX revamp master plan and implementation guidance.
...
Audit Logs
User Access
Legacy/non-routed surfaces that still exist in the repo:
Dashboard
Ledgers
Calendar
Settings
...
Do not remove any active capability.
Do not silently change any backend contract.
Do not change role permissions.
Do not change route semantics unless redirects/deep-link preservation are implemented.
Do not change business outcomes of create/edit/delete/report/export/payment/refund/audit flows.
Do not expose currently hidden backend/controller capabilities to end users unless explicitly required and parity-validated.
Do not delete legacy modules just to make the revamp easier.
Do not leave any migrated surface half-modernized.
Do not break existing filters, exports, status transitions, OCR prefills, linked account flows, or audit visibility rules.
Any code cleanup must be behavior-safe and covered by tests.
==================================================
...
Fix inbound refund identity mapping
...
Normalize PaymentMode contract
...
Fix booking update falsy-value semantics
...
Resolve broken bootstrap-owner script reference
...
Legacy Firebase ticket/settings handling
Do not leave broken Firebase imports and broken type paths contaminating the build graph
Either:
fully rehabilitate these flows onto supported dependencies/current architecture, or
isolate them behind a clearly marked legacy boundary so they no longer break type/lint/build confidence
Preserve code and intended capability footprint; do not casually delete
Type/lint debt containment
...
A concise architecture note explaining:
old IA
new IA
why hybrid IA was used
what routes were preserved
how legacy modules were handled
A parity matrix document listing for every module:
route
role access
...
tests covering it
A design system package/folder with documented primitives and tokens
A migration log showing:
what changed visually
what did not change behaviorally
what compatibility fixes were applied
where legacy boundaries remain
Automated tests and smoke scripts covering critical workflows
13. EXECUTION STYLE
...
Start by generating:
a repo-informed parity matrix,
the new IA proposal,
the design system structure,
the migration sequence,
the exact implementation plan by module,
...
When the user invokes `/ux-revamp` or describes a task the skill's description matches.
...
- Claude (if synced from `~/.claude/skills/ux-revamp`)
- HQ Project — landing page Skills section
- MD Project (md.sgnk.ai) — `Skills/Ux Revamp/`
- Obsidian Vault — `Skills/Ux Revamp/`
...
_Source: ~/.codex/skills/ux-revamp/SKILL.md_
_Category: General_
