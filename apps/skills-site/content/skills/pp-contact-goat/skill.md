...
When the user invokes `/pp-contact-goat` or describes a task the skill's description matches.
...
- Claude (if synced from `~/.claude/skills/pp-contact-goat`)
- HQ Project — landing page Skills section
- MD Project (md.sgnk.ai) — `Skills/Pp Contact Goat/`
- Obsidian Vault — `Skills/Pp Contact Goat/`
...
_Source: ~/.codex/skills/pp-contact-goat/SKILL.md_
