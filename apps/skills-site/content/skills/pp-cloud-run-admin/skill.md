...
When the user invokes `/pp-cloud-run-admin` or describes a task the skill's description matches.
...
- Claude (if synced from `~/.claude/skills/pp-cloud-run-admin`)
- HQ Project — landing page Skills section
- MD Project (md.sgnk.ai) — `Skills/Pp Cloud Run Admin/`
- Obsidian Vault — `Skills/Pp Cloud Run Admin/`
...
_Source: ~/.codex/skills/pp-cloud-run-admin/SKILL.md_
